      ...
      <div class="col-md-6">
        <div class="card mb-6">
          <div class="card-header header-elements">
            <h5 class="mb-0 me-2">Ubah Menu</h5>
          </div>
          <div class="card-body">
            <form action="{{ route('backpanel.website_settings.site_menus.update', ['site_menu' => $site_menu]) }}"
              method="POST">
              @csrf @method('PUT')
              <div class="mb-4">
                ...
                <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                  ...
                  @foreach ($types as $key => $value)
                    <option value="{{ $key }}" @selected($site_menu->type == $key)>
                      {{ $value }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="mb-4">
                ...
                <select class="form-select @error('parent_id') is-invalid @enderror" id="parent_id" name="parent_id">
                  <option value="" @selected($site_menu->parent_id == '')>Tanpa Parent</option>
                  @foreach ($parents as $row)
                    <option value="{{ $row->id }}" @selected($site_menu->parent_id == $row->id)>
                      {{ $row->title }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="mb-4">
                ...
                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title"
                  name="title" value="{{ old('title', $site_menu->title) }}" placeholder="masukan judul" required />
                @error('title')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              <div class="mb-4">
                ...
                <select class="form-select @error('link_type') is-invalid @enderror" id="link_type" name="link_type"
                  ...
                  @foreach ($link_types as $key => $value)
                    <option value="{{ $key }}" @selected($site_menu->link_type == $key)>
                      {{ $value }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="mb-4 d-none" id="external_url_container">
                ...
                <input type="text" class="form-control @error('external_url') is-invalid @enderror" id="external_url"
                  name="external_url" value="{{ old('external_url', $site_menu->value_external_url) }}"
                  placeholder="masukan URL external" />
                @error('external_url')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              <div class="mb-4">
                <label for="index" class="form-label">Index (urut dari terkecil) <span
                    class="text-danger">*</span></label>
                <input type="number" class="form-control @error('index') is-invalid @enderror"
                  id="index" name="index" value="{{ old('index', $site_menu->index) }}" placeholder="masukan index" required />
                @error('index')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
