...
@section('breadcrumb')
  <nav aria-label="breadcrumb" class="layout-navbar-user navbar-nav align-items-center ms-0 ms-md-3 me-3 me-xl-0">
    <ol class="breadcrumb breadcrumb-style1 mb-0">
      ...
      <li class="breadcrumb-item active">Ubah Postingan Media Sosial</li>
    </ol>
  </nav>
@endsection
      ...
      <div class="col-md-6">
        <div class="card mb-6">
          <div class="card-header header-elements">
            <h5 class="mb-0 me-2">Ubah Postingan Media Sosial</h5>
          </div>
          <div class="card-body">
            <form action="{{ route('backpanel.media.social.update', ['social_post' => $social_post]) }}" method="POST">
              @csrf @method('PUT')
              <div class="mb-4">
                <label for="name" class="form-label">Nama <span class="text-danger">*</span></label>
                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                  name="name" value="{{ old('name', $social_post->name) }}" placeholder="masukan judul" required />
                @error('name')
              ...
              <div class="mb-4">
                ...
                <textarea rows="5" class="form-control @error('script') is-invalid @enderror" id="script"
                  name="script" placeholder="masukan script" required>{{ old('script', $social_post->script) }}</textarea>
                @error('script')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              <div class="mb-4">
                ...
                <input type="date" class="form-control @error('published_at') is-invalid @enderror" id="published_at"
                  name="published_at" value="{{ old('published_at', $social_post->published_at->format('d/m/Y')) }}" placeholder="DD/MM/YYYY" required />
                @error('published_at')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
