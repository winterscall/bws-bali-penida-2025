    ...
    <div class="row g-6">
      <div class="col-md-6">
        <div class="card mb-6">
          ...
          <div class="card-body">
            <form action="{{ route('backpanel.website_settings.backlinks.update', ['backlink' => $backlink]) }}" method="POST">
              @csrf @method('PUT')
              <div class="mb-4">
                <label for="name" class="form-label">Nama <span class="text-danger">*</span></label>
                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                  name="name" value="{{ old('name', $backlink->name) }}" placeholder="masukan judul" required />
                @error('name')
              ...
              <div class="mb-4">
                ...
                <input type="url" class="form-control @error('url') is-invalid @enderror" id="url"
                  name="url" value="{{ old('url', $backlink->url) }}" placeholder="masukan URL" required />
                @error('url')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              <div class="mb-4">
                ...
                <input type="number" class="form-control @error('index') is-invalid @enderror" id="index"
                  name="index" value="{{ old('index', $backlink->index) }}" placeholder="masukan index" required />
                @error('index')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
